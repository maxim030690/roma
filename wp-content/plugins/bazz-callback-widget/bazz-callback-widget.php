...
define( 'BAZZ_WIDGET_VERSION', '3.12' );
...
function bazz_layout() { ?>
    ...
    <div class="bazz-widget">
        <div class="bazz-widget-button">
            <i></i>
            <i><div><?php _e( 'CALL NOW', 'bazz-callback-widget' ); ?></div></i>
        </div>
            ...
            <div class="bazz-widget-form-top">
                <label>
                    ...
                    <a href="javascript:void(0);"
                       class="bazz-widget-your-name"> <?php _e( 'you', 'bazz-callback-widget' ); ?></a>
					<?php if ( $bazz_options_arr['time'] == 0 ) { ?>
						<?php _e( 'back in the nearest future!', 'bazz-callback-widget' ); ?>
					<?php } else { ?>
						<?php _e( 'in', 'bazz-callback-widget' ); ?> 00:<span
                                class="bazz_time"><?php echo( $bazz_options_arr['time'] ); ?></span>
...
function bazz_menu_page() { ?>
    ...
    <form method="post" action="options.php" id="bazz-settings-form">
        ...
        <div class="option-work-time">
            ...
            <label><?php _e( 'The working day with', 'bazz-callback-widget' ); ?>
                <strong> <?php echo esc_attr( $bazz_options['work_time_start'] ); ?> </strong>
				<?php _e( 'h to', 'bazz-callback-widget' ); ?>
                <strong> <?php echo esc_attr( $bazz_options['work_time_end'] ); ?> </strong>
				<?php _e( 'h', 'bazz-callback-widget' ); ?></label><br>
            <em><?php _e( '*The time zone is using WordPress settings.', 'bazz-callback-widget' ); ?> 
			<?php _e( 'For now is ', 'bazz-callback-widget' );
			echo current_time( 'H:i', 0 ); ?></em>
        </div>
